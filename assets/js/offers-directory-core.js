...
//     return window.KSI18n?.get?.(key, fallback) || fallback;
//   }
...
//     selectEl.innerHTML =
//       `<option value="">${esc(t("offersDirectory.filters.allLocations", "Alle Standorte"))}</option>` +
//       cities.map((c) => `<option>${esc(c)}</option>`).join("");
//   }
...
//   function ageRange(o) {
//     const from = o?.ageFrom;
//     const to = o?.ageTo;

//     if (from && to) {
//       return tpl(t("offersDirectory.cards.ageRange", "{{from}}–{{to}} Jahre"), {
//         from: esc(from),
//         to: esc(to),
//       });
//     }
...
//     if (from) {
//       return tpl(t("offersDirectory.cards.fromAge", "ab {{age}} Jahre"), {
//         age: esc(from),
//       });
//     }
...
//     return days.length ? days.map(normDay).join(", ") : "";
//   }

//   function cardMetaParts(o) {
//     return [dayLabel(o), ageRange(o)].filter(Boolean);
//   }

//   function renderOfferCard(name, addr, metaParts = []) {
...
//         <h3 class="card-title">${esc(name)}</h3>
//         ${addr ? `<div class="offer-meta">${esc(addr)}</div>` : ""}
//         ${meta ? `<div class="offer-details">${meta}</div>` : ""}
//       </article>
//     `;
//   }
...
//   function renderEmpty(listEl) {
//     listEl.innerHTML = `
//       <li class="ks-offer ks-offer--empty">
//         <article class="card">
//           <h3 class="card-title">${esc(t("offersDirectory.cards.emptyTitle", "Keine Angebote gefunden."))}</h3>
//           <div class="offer-meta">${esc(t("offersDirectory.cards.emptyText", "Bitte passe deine Filter an."))}</div>
//         </article>
//       </li>
//     `;
//   }
...
//   function renderPowerGroups(listEl, groups) {
//     const label = t("offersDirectory.cards.offers", "Angebote");

//     listEl.innerHTML = groups
//       .map((g, i) => {
//         const details = [`${g.offers.length} ${esc(label)}`];

//         return `
//           <li class="ks-offer" data-offer-index="${i}" data-loc-key="${esc(g.key)}">
//             ${renderOfferCard(g.name, g.addr, details)}
//           </li>
//         `;
//       })
//       .join("");
...
//         return `
//           <li class="ks-offer" data-offer-index="${i}">
//             ${renderOfferCard(name, addr, cardMetaParts(o))}
//           </li>
//         `;
//       })
//       .join("");
...
//         return "6–25 Jahre";
//       case "CoachEducation":
//         return t("offersDirectory.cards.allAges", "alle Altersstufen");
//     }
//     return "";
//   }
...
//     const opts = collectHolidayOptions(arr, seasonValue);
//     sel.innerHTML =
//       `<option value="">${esc(t("offersDirectory.filters.allHolidayWeeks", "Alle Zeiträume"))}</option>` +
//       opts.map((k) => `<option value="${esc(k)}">${esc(k)}</option>`).join("");
//   }
