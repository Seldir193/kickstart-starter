...
(function () {
  ...
  function applyFrameHeight(panel, frame, height) {
    const maxPanelHeight = getMaxPanelHeight();
    const frameHeight = Math.min(height, maxPanelHeight);

    panel.style.maxHeight = `${maxPanelHeight}px`;
    frame.style.height = `${frameHeight}px`;
    panel.classList.add("ks-book-modal__panel--ready");
  }
  ...
  function applyCachedHeight(parts) {
    ...
    applyFrameHeight(parts.panel, parts.frame, cached);
  ...
  function handleHeightMessage(data) {
    const parts = getModalParts();
    if (!parts || parts.modal.hidden) return;
    if (parts.modal.dataset.ksHeightApplied === "1") return;
    if (!parts.panel || !parts.frame) return;
    if (!parts.frame.src || parts.frame.src === "about:blank") return;

    const height = Number(data.height);
    if (!Number.isFinite(height) || height <= 0) return;

    applyFrameHeight(parts.panel, parts.frame, height);
    saveHeight(height);
    parts.modal.dataset.ksHeightApplied = "1";
  }
