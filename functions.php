...
// Kontaktformular: für Gäste (nopriv) + Eingeloggte
...
function ks_handle_contact_form() {
  // Nonce prüfen – muss zu deinem Formular passen
  if (
...
function ks_contact_redirect($sent = '0') {
  $back = wp_get_referer();
  if (!$back) {
    // Fallback: deine Kontaktseite (Page-ID ggf. anpassen)
